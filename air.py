...
from telegram.ext import Updater, CommandHandler, CallbackQueryHandler, MessageHandler, Filters
    ...
    #------------------Deploy----------------------
def deploy_info(bot, country, space_list, card_id, lock_id, db):
    print('deploy_info')
    print(space_list) 
...
def deploy_cb(bot, query, query_list, db):
    if query_list[2] == 'confirm':
        bot.delete_message(chat_id = query.message.chat_id, message_id = query.message.message_id)
        deploy(bot, query_list[1], query_list[3], query_list[4], db)
        ...
        if query_list[2] == 'back':
            info = deploy_info(bot, query_list[1] , buffer.space_list, query_list[3], query_list[4], db)
            text = info[1]
            reply_markup = info[2]
        else:
...
def deploy(bot, active_country, space, card_id, db):
    ...
    if over_deploy_handler(bot, active_country, db):
        ...
        check_reposition(bot, db)
        function.updatesupply(db)
        lock_id = thread_lock.add_lock()
        status_handler.send_status_card(bot, active_country, 'Deploy/Marshal', lock_id, db, piece_id = piece[0][0], space_id = space, card_id = card_id)
        db.commit()
        
    #------------------Marshal----------------------
def marshal_info(bot, country, space_list, card_id, lock_id, db):
    print('marshal_info')
    print(space_list) 
    name_list = function.get_name_list(space_list, db)
    print(name_list)
    ...
    keyboard.append([InlineKeyboardButton('Pass', callback_data="['marshal', '{}', 'pass', {}]".format(country, card_id, lock_id))])
    reply_markup = InlineKeyboardMarkup(keyboard)
    return chat_id[0][0], text, reply_markup


def marshal_cb(bot, query, query_list, db):
    if query_list[2] == 'confirm':
        bot.delete_message(chat_id = query.message.chat_id, message_id = query.message.message_id)
        marshal(bot, query_list[1], query_list[3], query_list[4], db)
        thread_lock.release_lock(query_list[-1])
    elif query_list[2] == 'pass':
        bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id, text='Passed')
        thread_lock.release_lock(query_list[-1])
        ...
        if query_list[2] == 'back':
            info = marshal_info(bot, query_list[1] , buffer.space_list, query_list[3], query_list[4], db)
            text = info[1]
            reply_markup = info[2]
        else:
            location = db.execute("select name from space where spaceid = :id", {'id':query_list[2]}).fetchall()
...
def marshal(bot, active_country, space, card_id, db):
    space_info = db.execute("select distinct name, type from space where spaceid = :space;", {'space':space}).fetchall()
    active_country_name = db.execute("select name from country where id = :country;", {'country':active_country}).fetchall()
    group_chat = db.execute("select chatid from game;").fetchall()
    piece = db.execute("select min(pieceid) from piece where location = 'none' and control = :country and type = 'air';", {'country':active_country}).fetchall()
    ...
    #------------------------------------------Over_Deploy_Handler------------------------------------------
def over_deploy_handler(bot, active_country, db):
    ...
    for country in ['ge', 'it', 'jp', 'uk', 'su', 'us', 'fr', 'ch']:
        ...
        if len(no_control_air_list) > 0:
            for piece in no_control_air_list:
                lock_id = thread_lock.add_lock()
                space_list = function.control_supplied_space_list(country, db)
                info = reposition_info(country, piece[1], piece[0], lock_id, db)
                if info != None:
                    bot.send_message(chat_id = info[0], text = info[1], reply_markup = info[2])
                    thread_lock.thread_lock(lock_id)
                else:
                    battlebuild.remove(bot, country, piece[0] ,piece[1] , None, db)
    
def reposition_info(country, space_id, piece_id, lock_id, db):
    ...
    if len(space_list) > 0:
        ...
        keyboard.append([InlineKeyboardButton('Pass', callback_data="['reposition', '{}', 'pass', {}]".format(country, lock_id))])
        reply_markup = InlineKeyboardMarkup(keyboard)
        return chat_id[0][0], text, reply_markup
    else:
        return None
...
def reposition_cb(bot, query, query_list, db):
    if query_list[2] == 'confirm':
        bot.delete_message(chat_id = query.message.chat_id, message_id = query.message.message_id)
        ...
        bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id, text='Passed')
        battlebuild.remove(bot, query_list[1], query_list[4], query_list[3], None, db)
        thread_lock.release_lock(query_list[-1])
    else:
        if query_list[2] == 'back':
            info = reposition_info(bot, query_list[1] , buffer.space_list, query_list[3], db)
            text = info[1]
            reply_markup = info[2]
        else:
            location = db.execute("select name from space where spaceid = :id", {'id':query_list[2]}).fetchall()
...
def reposition(bot, active_country, space_id, piece_id, db):
    ...
    #------------------------------------------Air Defense------------------------------------------
    #TODO remove air
def air_defense(bot, handler_id, db):
    buffer.handler_list[handler_id].air_defense = True
    group_chat = db.execute("select chatid from game;").fetchall()
    text = function.countryid2name[buffer.handler_list[handler_id].passive_country_id] + " uesd Air Defense"
    bot.send_message(chat_id = group_chat[0][0], text = text)
    air_id = db.execute("select pieceid from piece where location = :location and control = :country and type = 'air';", {'location':buffer.handler_list[handler_id].space_id, 'country':buffer.handler_list[handler_id].passive_country_id}).fetchall()
    battlebuild.remove(bot, buffer.handler_list[handler_id].passive_country_id, air_id[0][0], buffer.handler_list[handler_id].space_id, None, db)
...
air_attack_list = []
class air_attack():
    def __init__(self, handler_id, lock_id):
        self.air_attack_id = len(air_attack_list)
        self.country = buffer.handler_list[handler_id].active_country_id
        #side = {'ge':'Axis', 'jp':'Axis', 'it':'Axis', 'uk':'Allied', 'su':'Allied', 'us':'Allied', 'fr':'Allied', 'ch':'Allied'}
        #self.space_list = list(set(function.control_air_space_list(buffer.handler_list[handler_id].active_country, db)) & set(within(buffer.handler_list[handler_id].active_country, [buffer.handler_list[handler_id].space_id], 1, db)))
    ...
    def air_attack_info(self, db):
        side = db.execute("select side from country where id = :country;", {'country':buffer.handler_list[self.handler_id].active_country_id}).fetchall()
        self.space_list = list(set(function.control_air_space_list(buffer.handler_list[self.handler_id].active_country_id, db)) & set(function.within(side[0][0], [buffer.handler_list[self.handler_id].space_id], 1, db)))
        #name_list = function.get_name_list(list(set(function.control_air_space_list(buffer.handler_list[self.handler_id].active_country_id, db)) & set(function.within(side[0][0], [buffer.handler_list[self.handler_id].space_id], 1, db))))
        name_list = function.get_name_list(self.space_list, db)
        chat_id = db.execute("select playerid from country where id = :country;", {'country':buffer.handler_list[self.handler_id].active_country_id}).fetchall()
    ...
    def air_attack_cb(self, bot, query, query_list, db):
        if query_list[1] == 'confirm':
            bot.delete_message(chat_id = query.message.chat_id, message_id = query.message.message_id)
            buffer.handler_list[query_list[-1]].air_defense = False
            #lock_id = thread_lock.add_lock()
            #battlebuild.self_remove_list.append(battlebuild.self_remove(buffer.handler_list[self.handler_id].passive_country_id, self.space_list, None, lock_id, 'air'))
            ...
            #lock_id = thread_lock.add_lock()
            group_chat = db.execute("select chatid from game;").fetchall()
            text = function.countryid2name[buffer.handler_list[self.handler_id].active_country_id] + " uesd Air Attack"
            bot.send_message(chat_id = group_chat[0][0], text = text)
            air_id = db.execute("select pieceid from piece where location = :space and control = :country and type = 'air';", {'space':self.space_id, 'country':buffer.handler_list[self.handler_id].active_country_id}).fetchall()
            battlebuild.remove(bot, buffer.handler_list[self.handler_id].active_country_id, air_id[0][0], self.space_id, None, db)
            battlebuild.remove(bot, buffer.handler_list[self.handler_id].active_country_id, buffer.handler_list[self.handler_id].piece_id, buffer.handler_list[self.handler_id].space_id, None, db)
            #space_name = db.execute("select distinct name from space where spaceid = :space;", {'space':buffer.handler_list[self.handler_id].space_id}).fetchall()
            #active_country_name = db.execute("select name from country where id = :country;", {'country':buffer.handler_list[self.handler_id].active_country_id}).fetchall()
            #country_name = db.execute("select name from country where id = (select control from piece where pieceid = :piece);", {'piece':buffer.handler_list[self.handler_id].piece_id}).fetchall()
            #db.execute("update piece set location = 'none' where pieceid = :piece;", {'piece':buffer.handler_list[self.handler_id].piece_id})
            #db.commit()
            #text = country_name[0][0] + " piece in " + space_name[0][0] + " is removed"
            #bot.send_message(chat_id = group_chat[0][0], text = text)  
            buffer.handler_list[self.handler_id].air_attack = True
            ...
            if query_list[1] == 'back':
                self.space_id = None
                info = self.air_attack_info(self, db)
                text = info[2]
                reply_markup = info[3]
            else:
