...
def send_status_card(bot, active_country_id, type_, lock_id, db, passive_country_id = None, card_id = None, space_id = None, piece_id = None):
    drawmap.drawmap(db)
    buffer.handler_list.append(buffer.handler(type_, active_country_id, lock_id, passive_country_id, card_id, space_id, piece_id))
    print("status_handler_id: " + str(len(buffer.handler_list)-1))
    handler_id = len(buffer.handler_list)-1
    ...
    for country in enemy_country_list[active_country_id]:
        info = info_list[type_](country, handler_id, db)
        if info[2] == None:
            buffer.handler_list[handler_id].no_respone[country] = True
        else:
            print('have - response ' + country)
            buffer.handler_list[handler_id].no_respone[country] = False
            pass_ = False
            status_message_id = bot.send_photo(chat_id = info[0], caption = info[1], reply_markup = info[2], parse_mode=telegram.ParseMode.HTML, photo=open('pic/tmp.jpg', 'rb'))
            buffer.handler_list[handler_id].message_id[country] = status_message_id.message_id
    if pass_:
        buffer.handler_list[handler_id].first = False
        buffer.handler_list[handler_id].one_side_pass = True
        ...
        for country in friendly_country_list[active_country_id]:
            ...
            if info[2] == None:
                ...
                buffer.handler_list[handler_id].message_id[country] = status_message_id.message_id
        if pass_:
            buffer.handler_list.pop(handler_id)
...
def send_status_card_cb(bot, query, query_list, db):
    handler_id = query_list[2]
    info_type = buffer.handler_list[handler_id].type_
    lock_id = buffer.handler_list[handler_id].lock_id
    if buffer.handler_list[handler_id].card_id != None:
        card_name = db.execute("select name from card where cardid = :cardid;",{'cardid':buffer.handler_list[handler_id].card_id}).fetchall()
    if query_list[3] == 'pass':
        buffer.handler_list[handler_id].first = False
        buffer.handler_list[handler_id].no_respone[query_list[1]] = True
        ...
        if not all([buffer.handler_list[handler_id].no_respone[country] for country in friendly_country_list[query_list[1]]]):
            if buffer.handler_list[handler_id].card_id != None:
        ...
        for country in friendly_country_list[query_list[1]]:
            message_id = buffer.handler_list[handler_id].message_id[country]
            ...
            if message_id != None:
                ...
                buffer.handler_list[handler_id].message_id[country] = None
        if buffer.handler_list[handler_id].one_side_pass:
            buffer.handler_list.pop(handler_id)
            thread_lock.release_lock(lock_id)
            return
        buffer.handler_list[handler_id].one_side_pass = True
        ...
        elif buffer.handler_list[handler_id].card_id != None:
        ...
        for country in friendly_country_list[query_list[1]]:
            if country != query_list[1]:
                ...
                if message_id != None:
                    ...
                    bot.delete_message(chat_id=chat_id[0][0], message_id = message_id)
                    buffer.handler_list[handler_id].message_id[country] = None
        buffer.handler_list[handler_id].one_side_pass = False
        #card execute
        if query_list[-1] == 'air_a':
            air_a_lock_id = thread_lock.add_lock()
            air.air_attack_list.append(air.air_attack(query_list[2], air_a_lock_id))
            print("air_attack_id: " + str(len(air.air_attack_list)-1))
            air_attack_id = len(air.air_attack_list)-1
            info = air.air_attack_list[air_attack_id].air_attack_info(db)
            bot.send_message(chat_id = info[0], text = info[1], reply_markup = info[2])
            thread_lock.thread_lock(air_a_lock_id)
        elif query_list[-1] == 'air_d':
            air.air_defense(bot, query_list[2], db)
        else:
            cardfunction.play_status(bot, query_list[-1], query_list[1], query_list[2], db)
        #card execute
        bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
        buffer.handler_list[handler_id].message_id[query_list[1]] = None
        buffer.handler_list[handler_id].first = False
        air.check_reposition(bot, db)
    else:
        if query_list[3] == 'back':
            info = info_list[info_type](query_list[1], handler_id, db)
            chat_id = info[0]
            text = info[1]
            reply_markup = info[2]
        else:
    ...
    if query_list[3] in ['pass', 'confirm']:
        ...
        for country in enemy_country_list[query_list[1]]:
            info = info_list[info_type](country, handler_id, db)
            message_id = buffer.handler_list[handler_id].message_id[country] 
...
'''def send_status_card_cb(bot, query, query_list, db):
    ...
    if query_list[3] == 'pass':
        ...
        used_card_name =  db.execute("select name from card where cardid = :card;",{'card':query_list[3]}).fetchall()
        if buffer.handler_list[handler_id].card_id != None:
            text = "<b>[" + card_name[0][0] + " - " + info_type + "]</b> - You used " + used_card_name[0][0] + ", processsing..."
        else:
            text = "<b>[" + info_type + "]</b> - You used " + used_card_name[0][0] + ", processsing..."
        ...
        buffer.handler_list[handler_id].one_side_pass = False
        #card execute
        cardfunction.play_status(bot, query_list[3], query_list[1], query_list[2], db)
        #card execute
    	bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
        buffer.handler_list[handler_id].message_id[query_list[1]] = None
    #enemy_country_list = db.execute("select id, playerid from country where side = (select enemy from country where id = :country);", {'country':query_list[1]}).fetchall()
    ...
    for country in enemy_country_list[query_list[1]]:
        ...
        if info[2] == None:                         #No response
            ...
            pass_ = False
            if message_id == None:
                status_message_id = bot.send_message(chat_id = info[0], text = info[1], reply_markup = info[2], parse_mode=telegram.ParseMode.HTML)
                buffer.handler_list[handler_id].message_id[country] = status_message_id
            else:
                bot.edit_message_text(chat_id = info[0], message_id = message_id, text = info[1], reply_markup = info[2], parse_mode=telegram.ParseMode.HTML)
    if pass_:
        if buffer.handler_list[handler_id].one_side_pass:
            buffer.handler_list.pop(handler_id)
            thread_lock.release_lock(lock_id)
    ...
    #------------------------------------------Status Handler Info------------------------------------------
        #--------------------------------------------Battle---------------------------------------------
def status_battle_handler(bot, active_country, passive_country, space, db):
...
def status_battle_handler_info(country, handler_id, db):
    ...
    active_country = buffer.handler_list[handler_id].active_country_id
    space_info = db.execute("select distinct spaceid, type, name from space where spaceid = :space", {'space':buffer.handler_list[handler_id].space_id}).fetchall()
    piece_info = db.execute("select control, location, supply, type from piece where pieceid = :piece;", {'piece':buffer.handler_list[handler_id].piece_id}).fetchall()
    ...
    keyboard = []
    if country == 'jp':
        response_count = db.execute("select count(*) from card where location = 'hand' and control ='sujp' and type = 'Response';").fetchall()[0][0]
    if country == 'su':
    ...
    if country == buffer.handler_list[handler_id].active_country_id:
        list1 = function.within(function.getside[buffer.handler_list[handler_id].active_country_id], [buffer.handler_list[handler_id].space_id], 1, db)
        list2 = function.control_air_space_list(buffer.handler_list[handler_id].active_country_id, db)
        print(buffer.handler_list[handler_id].air_defense)
        print(buffer.handler_list[handler_id].air_attack)
        if len([list1 and list2]) > 0  and buffer.handler_list[handler_id].air_defense and not buffer.handler_list[handler_id].air_attack:
            keyboard.append([InlineKeyboardButton('Air Attack', callback_data="['status_battle', '{}', {}, 'confirm', 'air_a']".format(country, handler_id))])
    if country == buffer.handler_list[handler_id].passive_country_id and buffer.handler_list[handler_id].first:
        if buffer.handler_list[handler_id].space_id in function.control_air_space_list(buffer.handler_list[handler_id].passive_country_id, db):
        ...
        for card in avaliable_card:
            ...
            if country == 'us':
                ...
                if card[0] == 346 and active_country == 'us' and space_info[0][1] == 'land' and space_info[0][0] in function.within('Allies', function.control_supplied_space_list('us', db, space_type = 'sea'), 1, db):
                    keyboard.append([InlineKeyboardButton(card[1], callback_data="['status_battle', '{}', {}, {}]".format(country, handler_id, card[0]))])
                if card[0] == 350 and piece_info[0][0] == 'us' and space_info[0][0] in function.supplied_space_list('us', db, space_type = 'sea'):
                    keyboard.append([InlineKeyboardButton(card[1], callback_data="['status_battle', '{}', {}, {}]".format(country, handler_id, card[0]))])
    ...
    return chat_id[0][0], text, reply_markup
        ...
        #--------------------------------------------Build---------------------------------------------
def status_build_handler(bot, active_country, db):
...
def status_build_handler_info(country, handler_id, db):
    ...
    return chat_id[0][0], text, reply_markup
        ...
        #--------------------------------------------Remove---------------------------------------------
def status_remove_handler_info(country, handler_id, db):
    ...
    if len(avaliable_card) > 0:
        ...
        for card in avaliable_card:
            ...
            if country == 'su':
                ...
                if card[0] == 292 and piece_info[0][0] == 'su' and space_info[0][0] == 24:
                    keyboard.append([InlineKeyboardButton(card[1], callback_data="['status_remove', '{}', {}, {}]".format(country, handler_id, card[0]))])
                if card[0] == 296 and piece_info[0][0] == 'su' and piece_count[0][0] == 0:
                    if ba_count > 0:
                        keyboard.append([InlineKeyboardButton(card[1], callback_data="['status_remove', '{}', {}, {}]".format(country, handler_id, card[0]))])
                    else:
    ...
    #----------------------Recuit-----------------------
def status_recuit_handler(bot, active_country, db):
...
def status_recuit_handler_info(country, handler_id, db):
    ...
    return chat_id[0][0], text, reply_markup
        ...
        #--------------------------------------------Deploy/Marshal---------------------------------------------
def status_deploy_handler_info(country, handler_id, db):
            ...
            if country == 'su':
                if card[0] == 300 and active_country == 'su':
                    if ba_count > 0:
                        ...
                        keyboard.append([InlineKeyboardButton(card[1] + ' - No Build Army in hand', callback_data="['status_after_play', '{}', {}, 'no_play', {}]".format(country, handler_id, card[0]))])
            if country == 'us':
...
def status_before_play_handler_info(country, handler_id, db):
...
def status_after_play_handler_info(country, handler_id, db):
    print('in status_after_play_handler_info - ' + country)
    s = {'ge': [], 'jp':[], 'it':[], 'uk':[227], 'su':[285], 'us':[], 'fr':[], 'ch':[]}
    chat_id = db.execute("select playerid from country where id = :id;",{'id':country}).fetchall()
    active_country = buffer.handler_list[handler_id].active_country_id
    played_card = buffer.handler_list[handler_id].card_id
...
def status_play_status_handler_info(country, handler_id, db):
...
def status_play_bolster_handler_info(country, handler_id, db):
    ...
    return chat_id[0][0], text, reply_markup
#----------------------Air Step-----------------------
def status_air_handler_info(country, handler_id, db):
    ...
    #----------------------Victory Step-----------------------
def status_victory_handler_info(country, handler_id, db):
            ...
            if country == 'jp':
                if card[0] == 122 and active_country == 'jp':
                    if response_count > 0:
                        ...
                        keyboard.append([InlineKeyboardButton("Bolster - " + card[1] + ' - No Response card in hand', callback_data="['status_play', '{}', {}, 'no_play', {}]".format(country, handler_id, card[0]))])
...
def status_extra_victory_point(country, db):
    ...
    #----------------------Draw Step-----------------------
def status_draw_handler_info(country, handler_id, db):
...
def status_discard_handler_info(country, handler_id, db):
...
def status_supply_handler_info(country, handler_id, db):
    ...
    #----------------------Economic Warfare-----------------------
def status_ew_handler(bot, cardid, active_country, passive_country, db):
    ...
    if len(avaliable_card) > 0:
        for card in avaliable_card:
            ...
            if card[0] == 46 and passive_country == 'ge':
                cardfunction.c46(bot, active_country, db)
...
def status_ew_handler_info(country, handler_id, db):
...
info_list = {'Battle':status_battle_handler_info,
             ...
             'Using a status':status_play_status_handler_info,
